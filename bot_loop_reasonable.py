...
#                  profit_target_pct: float = 2.5,        # ↑ Higher targets with better signals
        ...
        self.MAX_POSITION_PCT = 0.30  # 30% max position
...
class RSIStrategy(BaseStrategy):
    ...
    def __init__(self, config: StrategyConfig, session, market_data, logger):
        ...
        self.rsi_period = 14
        self.rsi_oversold_strong = 25
        self.rsi_oversold_moderate = 35
        self.rsi_overbought_moderate = 65
        self.rsi_overbought_strong = 75
...
class AccountManagerConfig:
    ...
    def __init__(self):
        ...
        self.MAX_POSITION_PCT = 0.30  # Max 30% of balance per position
...
class EnhancedAccountManager(AccountManager):
    ...
    def calculate_position_size_safe(self, symbol: str, entry_price: float, 
        ...
        - Max 30% of account per position
        """
        try:
            # Get current balance
            balance_info = self.get_account_balance()
            ...
            # 4. CRITICAL: Apply position value limit (30% of account max)
            position_value = position_size * entry_price
            max_allowed_value = available_balance * config.MAX_POSITION_PCT  # 0.30 = 30%
            
            if position_value > max_allowed_value:
                # Reduce position size to stay within limit
                position_size = max_allowed_value / entry_price
                position_value = position_size * entry_price
                logger.warning(f"⚠️ Position reduced to ${position_value:.2f} (30% limit)")
            ...
            if final_position_value > available_balance * 0.35:  # Absolute safety limit
...
class OrderManager:
    ...
    def validate_position_size(self, symbol: str, qty: float, price: float) -> bool:
        ...
        try: 
            ...
            # Hard limits
            if position_pct > 35:  # Never more than 20% in one position
                logger.error(f"❌ REJECTED: {symbol} position is {position_pct:.1f}% of account!")
                return False
        
            if position_value > balance * 0.30:  # Warn if over 30%
    ...
    def generate_all_signals(self, symbol: str) -> List[Dict]:
        ...
        for strategy_type, strategy in self.strategies.items():
            try:
                ...
                logger.info(f"[DEBUG] {symbol} {strategy.config.name}: signal={signal}, strength={strength:.3f}")
                # FIXME_DEBUG: Remove these lines when market data works 
